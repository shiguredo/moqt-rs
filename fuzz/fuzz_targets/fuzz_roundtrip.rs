...
fuzz_target!(|data: &[u8]| {
    ...
    // decode が返した Properties の生バイト (Properties Length varint 込み) をそのまま
    // encode へ渡し、Properties 検証経路を通した往復にする
    if let Ok((object, properties_data, _)) = SubgroupObject::decode(data, true) {
        let mut buf = Vec::new();
        let _ = object.encode(true, properties_data.as_deref(), &mut buf);
    }
