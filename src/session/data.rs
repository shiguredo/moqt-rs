...
pub(super) enum IncomingDataStream {
    ...
    /// 受信中の subgroup stream
    ///
    /// 状態は [`IncomingSubgroupStream`] が持つ。variant のフィールドを直接列挙する箇所を
    /// 減らすため、フィールドを専用の型にまとめている。
    Subgroup(IncomingSubgroupStream),
...
/// 受信中の subgroup stream の状態 ([`IncomingDataStream::Subgroup`])
///
/// SUBGROUP_HEADER の受信で確定する値と、Object の受信で更新する値を保持する。
/// variant のフィールドを直接列挙する箇所を [`IncomingDataStream`] のアクセサと本型の定義へ
/// 寄せるため、専用の型にしている。
#[derive(Debug, Clone, Copy)]
pub(super) struct IncomingSubgroupStream {
    /// header 時に紐づいた subscription の request_id
    pub(super) request_id: u64,
    /// SUBGROUP_HEADER の Track Alias
    pub(super) track_alias: u64,
    /// SUBGROUP_HEADER の Group ID
    pub(super) group_id: u64,
    /// 解決済みの Subgroup ID (SUBGROUP_ID_MODE `0b01` (FirstObjectId) は先頭 Object 受信まで `None`)
    ...
    pub(super) header_publisher_priority: Option<u8>,
    ...
    pub(super) end_of_group: bool,
    /// この stream で最後に受信した Object ID (FIN 時の Group 終端確定に使う)
    pub(super) last_object_id: Option<u64>,
    ...
    pub(super) last_attributed_request_id: Option<u64>,
    ...
    pub(super) timeout_override_request_id: Option<u64>,
}

impl IncomingDataStream {
    /// `Subgroup` variant の状態への参照を返す (他の variant では `None`)
    fn as_subgroup(&self) -> Option<&IncomingSubgroupStream> {
        match self {
            IncomingDataStream::Subgroup(stream) => Some(stream),
            _ => None,
        }
    }

    /// `Subgroup` variant の状態への可変参照を返す (他の variant では `None`)
    fn subgroup_mut(&mut self) -> Option<&mut IncomingSubgroupStream> {
        match self {
            IncomingDataStream::Subgroup(stream) => Some(stream),
            _ => None,
        }
...
/// subscription a received Object belongs to." の候補評価は header / object / datagram で
/// 共通だが、評価できるフィルタが異なる。SUBGROUP_HEADER の受信時点では Object ID と
/// Object Properties が未知のため、[`header_passes_filters`] による Group 単位の評価になる。
/// この違いを [`Session::evaluate_candidates`] の呼び出し側が組み立てる入力だけに閉じ込める。
#[derive(Debug, Clone)]
enum CandidateFilterInput<'a> {
    /// SUBGROUP_HEADER 受信時点の入力 (Object ID と Object Properties は未確定)
    Header {
        group_id: u64,
        subgroup_id: Option<u64>,
        publisher_priority: u8,
    },
    /// Object 受信時点の入力 (datagram は §11.2.1 のワイヤ構造に Subgroup ID が無いので `None`)
    Object(ObjectFilterInput<'a>),
}

impl CandidateFilterInput<'_> {
    /// 候補の subscription がフィルタを通過するかどうか
    fn passes(&self, subscription: &Subscription) -> bool {
        match self {
            CandidateFilterInput::Header {
                group_id,
                subgroup_id,
                publisher_priority,
            } => header_passes_filters(subscription, *group_id, *subgroup_id, *publisher_priority),
            CandidateFilterInput::Object(input) => object_passes_filters(subscription, input),
        }
    }
}

/// Track Alias の候補評価の結果 ([`Session::evaluate_candidates`])
#[derive(Debug, Clone, Copy, Default)]
struct CandidateEvaluation {
    /// 最初にフィルタを通過した受理対象の request_id
    ///
    /// キャンセル由来 `Terminated` の候補は受理対象にしない。
    matched_request_id: Option<u64>,
    /// 最初にフィルタを通過したキャンセル由来 `Terminated` の候補の request_id
    ///
    /// 合格した候補がキャンセル由来のみの場合に「不要 Object として破棄」する
    /// (draft §3.1.2) ため、受理対象が無いときの判定に使う。該当が無ければ `None`。
    cancelled_request_id: Option<u64>,
}

/// 受信 subgroup Object の帰属判定の結果
/// ([`Session::attribute_subgroup_object`])
#[derive(Debug, Clone, Copy)]
enum ObjectAttribution {
    /// 帰属先の subscription が確定した
    Accepted(u64),
    /// 帰属先が確定せず、Object を Application へ渡さない
    ///
    /// どの候補もフィルタを通過しなかった `FilteredOut` か、キャンセル由来候補のみが
    /// 合格した `Discarded` のいずれかである。
    Rejected(TrackDataAcceptance),
}
...
impl Session {
    ...
    pub fn recv_subgroup_header(
        ...
        match self.data_streams.incoming.get(&stream_id) {
            ...
            Some(IncomingDataStream::Subgroup(stream)) => {
                if self.is_cancelled_terminated_subscription(stream.request_id)
                    && self
                        .live_attribution(stream.last_attributed_request_id)
                        .is_none()
                {
                    ...
                    cancelled_request_id = Some(stream.request_id);
                } else {
                    return Err(SessionError::new(
                        SESSION_PROTOCOL_VIOLATION,
        ...
        // 候補ごとに異なる publisher_priority の解決と header 時点のフィルタ評価は
        // 入力の組み立てとして `evaluate_candidates` へ渡す。
        ...
        // 従来どおり受理する。
        let evaluation =
            self.evaluate_candidates(&candidates, |subscription| CandidateFilterInput::Header {
                group_id: header.group_id,
                subgroup_id,
                publisher_priority: subscription
                    .resolve_header_publisher_priority(header.publisher_priority),
            });
        let Some(request_id) = evaluation.matched_request_id else {
            if let Some(request_id) = evaluation.cancelled_request_id {
        ...
        self.data_streams.incoming.insert(
            stream_id,
            IncomingDataStream::Subgroup(IncomingSubgroupStream {
                request_id,
    ...
    pub fn recv_subgroup_object(
        ...
        self.require_established()?;
        // 段階 1: 受信 stream の状態を取得する
        // (破棄対象 stream への object 受信は no-op で吸収する)
        let Some(stream) = self.take_incoming_subgroup_object(stream_id, object.object_id)? else {
            return Ok(TrackDataAcceptance::Discarded);
        };

        // 段階 2: subgroup_id を解決する
        let resolved_subgroup_id = self.resolve_object_subgroup_id(stream_id, &stream, object)?;

        // 段階 3: 帰属判定
        let object_request_id = match self.attribute_subgroup_object(
            stream_id,
            &stream,
            object,
            resolved_subgroup_id,
        )? {
            ObjectAttribution::Accepted(request_id) => request_id,
            ObjectAttribution::Rejected(acceptance) => return Ok(acceptance),
        };

        // 段階 4: 受理後の状態更新
        // 段階 1 の写しは更新前の値を持つため、`first_object_received` が false なら
        // 今回の Object が Subgroup の先頭である
        let is_first_object = !stream.first_object_received;
        self.accept_subgroup_object(
            stream_id,
            &stream,
            object,
            object_request_id,
    ...
    /// [`recv_subgroup_object`](Self::recv_subgroup_object) の段階 1: 受信 stream の状態を取得する
    ///
    /// 破棄対象の stream (破棄対象の保持集合または `Discarded` variant) への Object 受信は
    /// no-op で吸収するため `Ok(None)` を返す。`Subgroup` variant 以外の状態への Object 受信は
    /// `PROTOCOL_VIOLATION` である。
    ///
    /// 返す写しは本段階の更新前の値を持つ。先頭 Object フラグが `false` なら、これから処理する
    /// Object が Subgroup の先頭である。wire 構造に属する更新 (先頭 Object フラグ・最終 Object ID・
    /// DATA_STREAM_TIMEOUT 用の activity) はフィルタ判定の結果に依存させない。
    fn take_incoming_subgroup_object(
        &mut self,
        stream_id: DataStreamId,
        object_id: u64,
    ) -> Result<Option<IncomingSubgroupStream>, SessionError> {
        // 破棄対象 stream への object 受信は no-op で吸収する (保持集合・ Discarded variant)
        if self.data_streams.discarded.contains_key(&stream_id) {
            return Ok(None);
        }
        let stream = match self.data_streams.incoming.get_mut(&stream_id) {
            Some(IncomingDataStream::Subgroup(stream)) => {
                let snapshot = *stream;
                // 先頭 Object フラグはフィルタ不通過でも立てる (wire 構造の整合を
                // フィルタ判定に依存させない)
                stream.first_object_received = true;
                ...
                stream.last_object_id = Some(object_id);
                snapshot
            }
            Some(IncomingDataStream::AwaitingHeader { .. }) => {
                return Err(SessionError::new(
    ...
    /// [`recv_subgroup_object`](Self::recv_subgroup_object) の段階 2: Object の subgroup_id を解決する
    ///
    ...
    /// 呼ばない。既存挙動を維持)。
    ///
    /// 戻り値は解決済み subgroup_id (Zero / Explicit は header 時点、FirstObjectId は本段階で確定)。
    fn resolve_object_subgroup_id(
        &mut self,
        stream_id: DataStreamId,
        stream: &IncomingSubgroupStream,
        object: &DecodedSubgroupObject,
    ) -> Result<Option<u64>, SessionError> {
        let resolve_subgroup_id = stream.subgroup_id.is_none();
        if resolve_subgroup_id {
            ...
            let resolved = self
                ...
                .incoming
                .get_mut(&stream_id)
                .and_then(IncomingDataStream::subgroup_mut);
            match resolved {
                Some(stream) => stream.subgroup_id = Some(object.object_id),
                None => {
                    return Err(SessionError::new(
                        SESSION_PROTOCOL_VIOLATION,
        ...
        let resolved_subgroup_id = self
            ...
            .incoming
            .get(&stream_id)
            .and_then(IncomingDataStream::as_subgroup)
            .and_then(|stream| stream.subgroup_id);
        if resolve_subgroup_id
            && let Some(resolved_subgroup_id) = resolved_subgroup_id
            && self.subscriptions.contains_key(&stream.request_id)
            && let Err(err) = self.peer_subgroups.record_priority(
                stream.track_alias,
                stream.group_id,
                resolved_subgroup_id,
    ...
    /// [`recv_subgroup_object`](Self::recv_subgroup_object) の段階 3: Object の帰属先を決める
    ///
    ...
    ///
    /// draft §12.1 (Malformed Tracks) 条件 2 (FIN 済み Subgroup への最終 Object ID より大きい
    /// Object) の検出はフィルタ判定より優先する。終端対象は帰属が確定していれば帰属先、
    /// 確定しなければ stream 所有者にする (Object を受理するはずの subscription を Malformed
    /// として cancel する)。この場合も subscription スコープの状態更新は行わない。
    fn attribute_subgroup_object(
        &mut self,
        stream_id: DataStreamId,
        stream: &IncomingSubgroupStream,
        object: &DecodedSubgroupObject,
        resolved_subgroup_id: Option<u64>,
    ) -> Result<ObjectAttribution, SessionError> {
        ...
        let candidates = self.resolve_peer_track_alias(stream.track_alias)?;
        if candidates.is_empty() {
            ...
            if let Some(reason) = fin_violation {
                ...
                self.terminate_malformed_track(stream.request_id, Some(stream_id), reason);
                return Err(err);
            }
            return Ok(ObjectAttribution::Rejected(TrackDataAcceptance::Discarded));
        }
        let evaluation = self.evaluate_candidates(&candidates, |subscription| {
            ...
            let publisher_priority =
                subscription.resolve_header_publisher_priority(stream.header_publisher_priority);
            CandidateFilterInput::Object(ObjectFilterInput {
                location: Location {
        ...
        if let Some(reason) = fin_violation {
            let err = SessionError::new(SESSION_PROTOCOL_VIOLATION, reason);
            let target_request_id = evaluation.matched_request_id.unwrap_or(stream.request_id);
            self.terminate_malformed_track(target_request_id, Some(stream_id), reason);
            return Err(err);
        }
        let Some(object_request_id) = evaluation.matched_request_id else {
            // 合格した候補がキャンセル由来のみなら不要 Object として破棄し、どの候補も
            // 通らなければフィルタ不通過として返す。どちらも subscription スコープの状態は
            // 更新しない (wire 構造と生存監視の更新は段階 1 で完了している)。
            return Ok(ObjectAttribution::Rejected(
                if evaluation.cancelled_request_id.is_some() {
        ...
        Ok(ObjectAttribution::Accepted(object_request_id))
    }

    /// [`recv_subgroup_object`](Self::recv_subgroup_object) の段階 4: 受理した Object を反映する
    ///
    /// subscription スコープの状態だけを更新する。wire 構造と生存監視の更新は段階 1、
    /// Malformed Track 条件 2 の判定は段階 3 で完了している。
    fn accept_subgroup_object(
        &mut self,
        stream_id: DataStreamId,
        stream: &IncomingSubgroupStream,
        object: &DecodedSubgroupObject,
        object_request_id: u64,
        resolved_subgroup_id: Option<u64>,
                ...
                crate::object_properties::ObjectProperties::decode(properties_bytes)
        {
                ...
                && let Some(subscription) = self.subscriptions.get_mut(&object_request_id)
            {
                ...
                if let Some(stream) = self
                    ...
                    .and_then(IncomingDataStream::subgroup_mut)
                {
                    stream.timeout_override_request_id = Some(object_request_id);
                }
            }
        }
        ...
        match self
            ...
            .and_then(IncomingDataStream::subgroup_mut)
        {
            Some(stream) => stream.last_attributed_request_id = Some(object_request_id),
            None => {
                return Err(SessionError::new(
                    SESSION_PROTOCOL_VIOLATION,
    ...
    pub fn recv_fetch_entry(&mut self, stream_id: DataStreamId) -> Result<(), SessionError> {
        ...
        match self.data_streams.incoming.get_mut(&stream_id) {
            ...
            Some(IncomingDataStream::Subgroup(_)) => Err(SessionError::new(
                SESSION_PROTOCOL_VIOLATION,
    ...
    pub fn recv_fetch_header(
        ...
        match self.data_streams.incoming.get(&stream_id) {
            ...
            Some(IncomingDataStream::Subgroup(_)) => {
                return Err(SessionError::new(
                    SESSION_PROTOCOL_VIOLATION,
    ...
    pub fn send_object_datagram(
        ...
        // "Any other value SHOULD be treated as a protocol error and the session SHOULD be
        //  closed with a PROTOCOL_VIOLATION (Section 12.2)."
        // 未知 status は wire 生成前 (フィルタ評価と最大位置更新より前) に拒否する。
        // 判定は encoder (`ObjectDatagram::encode`) と同じ validate_object_status を使う。
    ...
    ///
    /// 手組みの `ObjectDatagram` を渡した場合でも、wire 経路 (`ObjectDatagram::decode`) と同じ
    /// protocol 検証 (draft §11.1.2 Object Status / §11.1.3 Object Properties / §11.2.1 Object
    /// Datagram) を行い、違反時は `SESSION_PROTOCOL_VIOLATION` でセッションを閉じる。
    ///
    /// Object Payload 長の規則 (status がある場合は payload を保持しない、status がなく
    /// zero-length payload を禁止する) は `ObjectDatagram` が payload を保持しないため本 API では
    /// 検証しない。この規則は wire decode 層の責務である。
    pub fn recv_object_datagram(
        &mut self,
        datagram: &ObjectDatagram,
    ) -> Result<TrackDataAcceptance, SessionError> {
        self.require_established()?;
        // draft-ietf-moq-transport-21 §11.1.2 (Object Status) / §11.1.3 (Object Properties) /
        // §11.2.1 (Object Datagram):
        // wire 経路 (`recv_datagram` → `ObjectDatagram::decode`) が行う検証を、decode 済みまたは
        // 手組みの `ObjectDatagram` を受け取る本 API の入口でも同じ順序で行う。違反時は
        // wire 経路と同じ `SESSION_PROTOCOL_VIOLATION` でセッションを閉じる。
        // Track Alias 解決と状態更新より前に検証し、拒否時に状態を汚染しない。
        //
        // 検証しない項目: Object Payload 長の規則 (status ありなら payload なし、status なしの
        // zero-length の禁止) は `ObjectDatagram` が payload を保持しないため本 API では判定できない。
        // payload 長の整合は wire decode 層 (`ObjectDatagram::decode`) の責務である。
        let validation_error = validate_object_datagram_type(
            // struct から wire の Type Flags を復元する (decode 済みなら decode 時の値と一致する)
            (if datagram.status.is_some() {
                0x20
            } else {
                0x00
            }) | (if datagram.end_of_group { 0x02 } else { 0x00 }),
        )
        .err()
        .or_else(|| {
            datagram
                .status
                .and_then(|status| crate::stream::validate_object_status(status).err())
        })
        .or_else(|| {
            datagram
                .properties_data
                .as_deref()
                .and_then(|data| validate_datagram_properties_blob(data).err())
        })
        .or_else(|| {
            // draft §11.1.3 (Object Properties): 非 Normal status に Properties は不可 (MUST close)
            if datagram.properties_data.is_some()
                && datagram
                    .status
                    .is_some_and(|status| status != OBJECT_STATUS_NORMAL)
            {
                Some(crate::error::MessageError::ProtocolViolation(
                    "properties on non-Normal status object is not allowed",
                ))
            } else {
                None
        ...
        if let Some(err) = validation_error {
            let session_error = session_error_from_data_message(err);
            self.fail(session_error.clone());
    ...
    pub fn report_mid_object_fin(&mut self, stream_id: DataStreamId) -> Result<(), SessionError> {
        ...
        // 判定と後始末は `discard_cancelled_subgroup_stream` に集約している
        let removed_subgroup = removed
            .as_ref()
            .and_then(IncomingDataStream::as_subgroup)
            .copied();
        let discarded = matches!(removed, Some(IncomingDataStream::Discarded { .. }))
            || removed_subgroup
                .is_some_and(|stream| self.discard_cancelled_subgroup_stream(&stream));
        if discarded {
    ...
    ///
    /// FIN で閉じた stream に対して同じ id の終端をもう一度通知しても no-op で吸収する。
    /// draft-ietf-moq-transport-21 §11.3.2 (Closing Subgroup Streams) が「FIN を受信した
    /// アプリケーションはその後の reset を無視してよい」と定めており、QUIC では FIN 送信後も
    /// RESET_STREAM を送れるためである。保持期間は `peer_alias_retention_ms`
    /// (`tick_discarded_data_stream_ids` が掃除する)。
    ///
    ...
    pub fn recv_data_stream_closed(
        ...
        let Some(stream) = self.data_streams.incoming.remove(&stream_id) else {
            ...
            // FIN で閉じた stream に対して、その後に届く RESET_STREAM は no-op で吸収する。
            // draft-ietf-moq-transport-21 §11.3.2 (Closing Subgroup Streams):
            // "A sender might send all objects in a Subgroup and the FIN on a QUIC stream, and
            // then reset the stream. In this case, the receiving application would receive the
            // FIN if and only if all objects were received. If the application receives all data
            // on the stream and the FIN, it can ignore any subsequent reset."
            // QUIC では FIN 送信後も RESET_STREAM を送れるため、この順序は正当である。
            // FIN の重複通知は I/O 層の誤りであり、従来どおり unknown stream id として
            // 検出する (吸収するのは reset のみ)。
            if matches!(end, RequestStreamEnd::Reset { .. })
                && self.data_streams.fin_closed.contains_key(&stream_id)
            {
                return Ok(());
            }
            return Err(SessionError::new(
        ...
        self.timing.data_stream_last_activity_ms.remove(&stream_id);
        // FIN で閉じた id を保持し、以後の終端通知 (RESET_STREAM) を no-op で吸収する。
        // QUIC の stream id は再利用されないため、保持中に同一 id の正当なイベントが
        // 届くことはない。掃除は tick_discarded_data_stream_ids が行う。
        if matches!(end, RequestStreamEnd::Fin) {
            self.retain_fin_closed_stream_id(stream_id);
        }
        match stream {
            IncomingDataStream::AwaitingHeader { .. } => Ok(()),
            IncomingDataStream::Subgroup(stream) => {
                ...
                // 破棄分岐の判定と後始末は `discard_cancelled_subgroup_stream` に集約している
                if self.discard_cancelled_subgroup_stream(&stream) {
                    if let Some(subgroup_id) = stream.subgroup_id {
                    ...
                    && !self.is_cancelled_terminated_subscription(attributed_request_id)
                {
                    if let Some(last_object_id) = stream.last_object_id {
                        ...
                        {
                            subscription.ended_groups.insert(stream.group_id, 0);
                        }
                    }
                }
                if let Some(subgroup_id) = stream.subgroup_id {
                    ...
                    self.remove_subgroup_delivery_timeout_override(
                        stream.request_id,
                        stream.timeout_override_request_id,
                        stream.group_id,
                        subgroup_id,
                    );
                    match end {
                        RequestStreamEnd::Fin => {
                            ...
                            if let Err(err) = self.peer_subgroups.mark_fin(
                                ...
                                // 永久に false になるのを防ぐ)
                                let _ = self.note_incoming_stream_closed(stream.request_id);
                ...
                self.close_incoming_stream_or_fail(stream.request_id)
            }
            ...
            IncomingDataStream::Subgroup(stream) => {
                ...
                if self.discard_cancelled_subgroup_stream(&stream) {
                    if let Some(subgroup_id) = stream.subgroup_id {
                        ...
                        self.peer_subgroups.mark_stop_sending(
                            stream.track_alias,
                            stream.group_id,
                            subgroup_id,
                        );
                ...
                IncomingDataStream::Subgroup(stream) => {
                    if stream.request_id == request_id {
                        ...
                        let migration_target = live_attributed_request_id(
                            &self.subscriptions,
                            stream.last_attributed_request_id,
                        )
                        .filter(|&owner| owner != request_id)
                        .or_else(|| {
                            self.aliases
                                .peer_publisher_aliases
                                .get(&stream.track_alias)
                                .and_then(|candidates| {
                        ...
                        match migration_target {
                            Some(owner) => {
                                stream.request_id = owner;
                                // 回収済み・キャンセル済みの request_id を override 所有者と
                                // して残さない
                                stream.timeout_override_request_id = live_attributed_request_id(
                                    &self.subscriptions,
                                    stream.timeout_override_request_id,
                                );
                            ...
                            None => {
                                ...
                                if let (Some(override_request_id), Some(subgroup_id)) =
                                    (stream.timeout_override_request_id, stream.subgroup_id)
                                {
    ...
    /// `open_incoming_subgroup_count` をデクリメントする。`Discarded` variant の終端
    /// (`retain_discarded_stream_id`) は `note_incoming_stream_closed` を呼ばないため、
    /// 置き換え時点で補正しないと計数が張り付いたまま残り、`cleanup_ready()` が
    /// `open_incoming_subgroup_count == 0` を要求するために `forget_subscription` が
    /// 永久に不可能になる (subscription リーク)。
    fn register_discarded_stream(&mut self, stream_id: DataStreamId, request_id: Option<u64>) {
        let existing_subgroup = self
            .data_streams
            .incoming
            .get(&stream_id)
            .and_then(IncomingDataStream::as_subgroup)
        ...
        self.timing.data_stream_last_activity_ms.remove(&stream_id);
        if let Some(existing_subgroup) = existing_subgroup {
            ...
            self.cleanup_discarded_subgroup_stream(&existing_subgroup);
        }
    }

    /// 破棄対象の subgroup stream が破棄分岐に入るかどうかを判定し、入る場合は後始末する
    ///
    /// 破棄分岐へ入る条件は「キャンセル由来 `Terminated` に属し、現在も生きた Object の
    /// 帰属先が無い」ことである。帰属実績の参照先が生きている stream は帰属先 subscription
    /// にとって生きた stream であり、所有者のキャンセルだけを理由に破棄しない
    /// (draft §3.1 (Subscriptions) のフィルタ再適用で Object を受理した実績が根拠)。
    /// 帰属先が forget 済みの場合も死んだ id を根拠に session を閉じず、破棄対象として吸収する。
    ///
    /// 破棄対象なら `true` を返し、
    /// [`cleanup_discarded_subgroup_stream`](Self::cleanup_discarded_subgroup_stream) で
    /// 会計と per-subgroup delivery timeout override を戻す。wire 構造の終端
    /// (`peer_subgroups` への FIN / RESET / STOP_SENDING の記録) は終端の種類で異なるため
    /// 呼び出し側の責務である。
    fn discard_cancelled_subgroup_stream(&mut self, stream: &IncomingSubgroupStream) -> bool {
        if !self.is_cancelled_terminated_subscription(stream.request_id)
            || self
                .live_attribution(stream.last_attributed_request_id)
                .is_some()
        ...
        self.cleanup_discarded_subgroup_stream(stream);
        true
    }

    /// 破棄する subgroup stream の後始末を行う
    ///
    /// 受信 stream の会計を戻し (この経路は `register_discarded_stream` を通らないため、
    /// 計数リークで `cleanup_ready` が永久に false になるのを防ぐ)、per-subgroup delivery
    /// timeout override を stream 所有者と登録先 (先頭 Object の帰属先) の両方から削除する。
    ///
    /// 破棄分岐へ入る条件の判定は
    /// [`discard_cancelled_subgroup_stream`](Self::discard_cancelled_subgroup_stream) に置く。
    /// 本関数を条件判定なしで呼ぶのは、既存 `Subgroup` variant を無条件に破棄する
    /// [`register_discarded_stream`](Self::register_discarded_stream) だけである。
    fn cleanup_discarded_subgroup_stream(&mut self, stream: &IncomingSubgroupStream) {
        let _ = self.note_incoming_stream_closed(stream.request_id);
        if let Some(subgroup_id) = stream.subgroup_id {
            self.remove_subgroup_delivery_timeout_override(
                stream.request_id,
    ...
    /// FIN で閉じた stream id を保持集合へ登録する
    ///
    /// 保持期間は登録時点から `peer_alias_retention_ms`（破棄対象 stream と同じ長さ）。
    /// 既に登録済みの id は期限を更新しない。
    /// `discarded` と異なり受信そのものは吸収しない (終端通知の重複のみを吸収する)。
    fn retain_fin_closed_stream_id(&mut self, stream_id: DataStreamId) {
        self.data_streams
            .fin_closed
            .entry(stream_id)
            .or_insert_with(|| {
    ...
    pub(super) fn tick_discarded_data_stream_ids(&mut self, now_ms: u64) {
        ...
        // FIN で閉じた stream id の保持期間も同じ tick で進める
        for timer in self.data_streams.fin_closed.values_mut() {
            timer.tick(now_ms);
        }
        self.data_streams
            .fin_closed
    ...
    /// 呼び出し側は [`evaluate_candidates`](Self::evaluate_candidates) で候補を絞り込み、
    ...
    /// 候補 subscription を順に評価し、最初に通過した subscription を選ぶ
    ///
    ...
    ///
    /// 候補順にフィルタを評価し、最初に通過した（キャンセル由来 `Terminated` でない）
    /// subscription を採用する。キャンセル由来 `Terminated` の候補は受理対象から除外しつつ
    /// フィルタ評価だけは実行し、合格した候補がキャンセル由来のみの場合は
    /// [`CandidateEvaluation::cancelled_request_id`] で判別できるようにする
    /// (draft §3.1.2 の「不要 Object として破棄」)。
    ///
    /// header / object / datagram の差は `build_input` が組み立てる
    /// [`CandidateFilterInput`] だけに閉じ込める。候補ごとに解決が必要な値
    /// (Publisher Priority など) も `build_input` が担う。
    fn evaluate_candidates<'a, F>(
        &self,
        candidates: &[u64],
        mut build_input: F,
    ) -> CandidateEvaluation
    where
        F: FnMut(&Subscription) -> CandidateFilterInput<'a>,
    {
        let mut evaluation = CandidateEvaluation::default();
        for &candidate_id in candidates {
            let Some(subscription) = self.subscriptions.get(&candidate_id) else {
                continue;
            };
            if !build_input(subscription).passes(subscription) {
                continue;
            }
            if is_cancelled_terminated(subscription) {
                // 複数合格時は最初に合格した候補を保持する (上書きしない。後続の Established
                // 候補の評価を失わないため break もしない)
                if evaluation.cancelled_request_id.is_none() {
                    evaluation.cancelled_request_id = Some(candidate_id);
                }
            } else {
                evaluation.matched_request_id = Some(candidate_id);
                break;
            }
        }
        evaluation
