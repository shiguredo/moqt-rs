...
use super::delivery::{
    compute_effective_delivery_timeout_ms, set_subscription_expires,
...
impl Session {
    ...
    pub fn send_subscribe_ok(
        ...
        // "It contains the largest Location ... in the Track observed by the sending
        //  endpoint ... If Objects have been published on this Track the Publisher MUST
        //  include this parameter."
        // LARGEST_OBJECT は Track 単位の値であるため、同一 Track の publisher 役
        // subscription 群の最大値 (publisher_track_largest) を使う。新規 subscription は
        // 観測値を持たないが、同じ Track の別 subscription で publish 済みなら値を載せる。
        // publisher_track_largest は self を不変借用するため、subscription の可変借用より前に求める。
        let track_largest = match self.subscriptions.get(&request_id) {
            Some(subscription) => self
                .publisher_track_largest(&subscription.track_namespace, &subscription.track_name),
            None => None,
        };
        ...
        if let Some(ref loc) = track_largest {
    ...
    pub fn send_publish_state_notify(
        ...
        // draft-ietf-moq-transport-21 §9.20.18 (LARGEST OBJECT Parameter):
        // LARGEST_OBJECT は Track 単位の値であるため、同一 Track の publisher 役
        // subscription 群の最大値 (publisher_track_largest) を使う。
        // self の可変借用より前に求める (不変借用が必要なため)。
        let track_largest =
            self.publisher_track_largest(&subscription.track_namespace, &subscription.track_name);
