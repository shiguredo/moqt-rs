...
pub(super) struct DataStreamState {
    ...
    /// FIN で閉じた受信 data stream id の保持集合
    ///
    /// draft-ietf-moq-transport-21 §11.3.2 (Closing Subgroup Streams): "A sender might send
    /// all objects in a Subgroup and the FIN on a QUIC stream, and then reset the stream.
    /// ... If the application receives all data on the stream and the FIN, it can ignore any
    /// subsequent reset."
    ///
    /// QUIC では FIN 送信後も RESET_STREAM を送れるため、FIN で閉じた id を登録時点から
    /// `peer_alias_retention_ms` の期間だけ保持し、同じ id の終端通知を no-op で吸収する。
    /// [`Self::discarded`] と分けているのは、破棄対象 stream の受信を広く吸収する
    /// `discarded` に対し、本集合は終端通知の重複吸収だけを目的とするためである
    /// (直列化途中の FIN を検出する `report_mid_object_fin` の判定を弱めない)。
    pub(super) fin_closed: HashMap<DataStreamId, super::types::DeadlineTimer>,
}
...
impl Session {
    ...
    pub fn recv_request_stream_closed(
        ...
        // SHOULD then send a FIN on its direction, gracefully closing the stream."
        // requester 側で responder の FIN を受けたら、送信方向を閉じるよう I/O 層へ依頼する。
        // 実際に FIN するかは I/O 層が判断する (既に FIN 済みなら no-op)。
        if matches!(end, RequestStreamEnd::Fin) && self.is_local_requester(request_id, kind) {
            self.events
    ...
    ///
    /// draft-ietf-moq-transport-21 §6.4.2.2 (Graceful Request Stream Closure) の
    /// 「requester は FIN を返す SHOULD」を適用する対象かを判定するために使う。
    /// 継続中の request が対象であり、既に `forget` 済みの request は判定できないため false を
    /// 返す (その場合 I/O 層は既に stream を閉じている)。
    ///
    /// PUBLISH 起点の subscription は対象外とする。publisher は購読の終了時に PUBLISH_DONE を
    /// 送ってから FIN する必要があり (同節 "the publisher of an Established subscription MUST
    /// send PUBLISH_DONE, before sending a FIN")、responder の FIN を受けた時点で送信方向を
    /// 閉じると PUBLISH_DONE を送れなくなるためである。実際に PUBLISH_DONE と共に FIN する
    /// (`send_publish_done` の `fin`)。
    fn is_local_requester(&self, request_id: u64, kind: RequestKind) -> bool {
        match RequestTable::from_kind(kind) {
            // SUBSCRIBE は subscriber が要求を開始した側である
            // (draft-ietf-moq-transport-21 §3.1 (Subscriptions))。
            RequestTable::Subscription => match kind {
                RequestKind::Subscribe => self
                    .subscriptions
                    .get(&request_id)
                    .is_some_and(|s| s.my_role == TrackRole::Subscriber),
                _ => false,
            },
            // FETCH は subscriber 役が要求を開始した側である
            // (draft-ietf-moq-transport-21 §9.11 (FETCH))。
            RequestTable::Fetch => self
                .fetches
                .get(&request_id)
