...
use shiguredo_moqt::parameter::SetupOptions;
use shiguredo_moqt::session::core::Session;
use shiguredo_moqt::session::types::{SessionError, SessionEvent, Transport};
...
型ごとのシグネチャは次のとおり (一般化しない。実際の `src/stream/` と一致させること)。

```rust,ignore
// ヘッダ (draft-ietf-moq-transport-21 §11.3.1 / §11.4.1)
fn SubgroupHeader::encode(&self) -> Vec<u8>
fn SubgroupHeader::decode(buf: &[u8]) -> Result<(SubgroupHeader, usize), MessageError>
fn FetchHeader::encode(&self) -> Vec<u8>
fn FetchHeader::decode(buf: &[u8]) -> Result<(FetchHeader, usize), MessageError>

// Object (Properties の有無は引数で渡す)
fn SubgroupObject::encode(
    &self,
    has_properties: bool,
    properties_data: Option<&[u8]>,
    buf: &mut Vec<u8>,
) -> Result<(), MessageError>
fn SubgroupObject::decode(
    buf: &[u8],
    has_properties: bool,
) -> Result<(SubgroupObject, Option<Vec<u8>>, usize), MessageError>

fn FetchStreamEntry::encode(
    &self,
    properties_data: Option<&[u8]>,
    prior_context: FetchPriorContext,
    buf: &mut Vec<u8>,
) -> Result<(), MessageError>
fn FetchStreamEntry::decode(
    buf: &[u8],
    prior_context: FetchPriorContext,
) -> Result<(FetchStreamEntry, usize), MessageError>

fn FetchStreamObject::encode(
    &self,
    properties_data: Option<&[u8]>,
    prior_context: FetchPriorContext,
    buf: &mut Vec<u8>,
) -> Result<(), MessageError>
// FetchStreamObject に公開の decode は無い。`FetchStreamEntry::decode` が
// flags を読んでから内部の decode_after_flags を通す

// Datagram (1 datagram = 1 バッファ)
fn ObjectDatagram::encode(&self) -> Result<Vec<u8>, MessageError>
fn ObjectDatagram::decode(buf: &[u8]) -> Result<(ObjectDatagram, usize), MessageError>
```
...
fn new_with_group_order(request_id: u64, group_order: u8) -> Result<FetchStreamEncoder, MessageError>
...
fn example(bytes: &[u8]) -> Result<(), MessageError> {
    // Full / Delta を自動判別
...
型は `MsfCatalogDocument { Full(MsfCatalog), Delta(MsfDeltaUpdate) }`、要素は `MsfCatalog` /
`MsfDeltaOperation` / `MsfDeltaUpdate` / `MsfTrack` / `MsfCloneTrack` / `MsfRemoveTrack` /
`MsfInitData` / `MsfBuffers` / `MsfPackaging`、定数は `MSF_CATALOG_TRACK_NAME` / `MSF_VERSION`。
...
fn remove_track_alias(&mut self, track_alias: u64)
fn record_priority(
    &mut self,
    track_alias: u64,
    group_id: u64,
