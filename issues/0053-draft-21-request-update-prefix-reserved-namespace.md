...
- `handle_peer_subscribe_namespace` (SUBSCRIBE_NAMESPACE) と `handle_peer_subscribe_tracks` (SUBSCRIBE_TRACKS) は、`.` を `"reserved single-period namespace"`、`.session` を `"session-level track/namespace does not exist"` として `DOES_NOT_EXIST` の REQUEST_ERROR で拒否する。
- `handle_update_for_namespace_subscription` / `handle_update_for_track_subscription` は role / state と prefix overlap だけを検証し、`parameters.track_namespace_prefix()` をそのまま `entry.prefix` へ書き込む。`request_id` の種別ごとの REQUEST_UPDATE 経路で予約名前空間の検証がない。
...
- 受信側は prefix の更新適用前に初回と同じ予約名前空間検証を置く。`.` / `.session` は `DOES_NOT_EXIST` の REQUEST_ERROR で拒否し、prefix を反映しない。拒否時の bidi stream の扱いは既存の prefix overlap 拒否と揃える。
- 送信側も同じ条件でローカル拒否する。prefix を「確定待ち」として保持するシグネチャ変更は open issue 0017 が扱うため、本 issue では検証の追加に絞り、0017 と衝突する場合は 0017 の実装時に統合する。
...
- 送信側 API が `.` / `.session` への prefix 更新を拒否すること
