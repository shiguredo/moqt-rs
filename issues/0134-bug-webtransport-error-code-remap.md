...
本 issue が対象とするのは endpoint が送出する RESET_STREAM / STOP_SENDING のエラーコードであり、peer から受信した STOP_SENDING のコード変換は対象外とする (設計方針を参照)。
...
- 受信時は `fn wt_to_moqt_code(http3_code: u64) -> Option<u32>` (`ApplicationErrorCode::from_http3_code` を呼ぶ) と、`RequestStreamEnd::Reset` に入れる値を決める `fn wt_reset_error_code(http3_code: u64) -> u64` を `webtransport.rs` に置く。
  `wt_reset_error_code` は remap できれば MOQT のコードを返し、できなければ `tracing::warn!` で生値をログに残したうえで wire の HTTP/3 コードをそのまま返す。warn は「WT_APPLICATION_ERROR の範囲外」と「範囲内だが予約コードポイント (0x1f * N + 0x21)」を別の文言で区別する。
  `RequestStreamEnd::Reset` の組み立ては `fn wt_reset_stream_end(http3_code: u64) -> RequestStreamEnd` に切り出し、`WtRecvStream::recv_chunk` はこの戻り値を
  `error_code` に入れる (接続なしで `RequestStreamEnd::Reset` の組み立てをテストできるようにするため)。`None` を返す値 (WT_SESSION_GONE などのプロトコルコードや予約コードポイント) には §4.4 の次の SHOULD が適用されるが、後述の理由でこれには従わない (逸脱である)。
...
- 送信の remap は `fn moqt_application_error(error_code: u64) -> Result<s2n_quic::application::Error>` に閉じ、`WtSendStream::reset` / `WtRecvStream::stop_sending` の 2 サイトから呼ぶ。生の `s2n_quic::application::Error::new` を変換関数以外から呼ばない。
- 受信は `WtRecvStream::recv_chunk` のエラー分岐を `fn wt_recv_end(error: s2n_quic::stream::Error) -> Result<RecvChunk>` に切り出す。
  `RequestStreamEnd::Reset` の組み立ては `fn wt_reset_stream_end(http3_code: u64) -> RequestStreamEnd`、コードの変換は `fn wt_reset_error_code(http3_code: u64) -> u64` が担う。
  `recv_chunk` が `wt_recv_end` を通ることは clippy の dead_code 検査 (`-D warnings`) とレビューで守る (ハンドルが無いためテストでは固定できない)。`transport.rs` の WebTransport 分岐に変換を散らさない。
- peer から受信する STOP_SENDING の error code は対象外とする。s2n-quic は受信した STOP_SENDING を送信側の `StreamError::StreamReset` に変換するが、この example はその経路で MOQT のコードへ戻す処理を持たず、§4.4 の受信側 (RESET_STREAM と STOP_SENDING の両方が対象) のうち片側だけを実装した状態になる。完全に満たすには別途対応が必要になる
...
- 送信側 / 受信側の変換が §4.4 のアンカー (0x00000000 が `0x52e4a40fa8db`、0xffffffff が `0x52e5ac983162`) と一致することをリテラルで固定するテストが追加されていること
- WebTransport 経路で送るエラーコードが remap され、受信側で元の MOQT のコードに戻ることを `webtransport.rs` の `#[cfg(test)] mod tests` で固定するテストが追加されていること (I/O ハンドルを持たない変換関数を直接呼ぶ)
  ...
  - 32 ビットに収まらない値 (`u32::MAX + 1` と `u64::MAX`) を送信側の変換関数へ直接渡すとエラーになること (切り捨てないこと)。MOQT §13 の greasing 値は 32 ビットを超える値を取り得るため、この制限をコメントに記録すること
  - 予約コードポイントの warn が範囲外の warn と区別できる文言になり、どちらも生値を含むこと (ログを記録するサブスクライバで確認する)
  - `wt_recv_end` (= `recv_chunk` のエラー分岐) が `RESET_STREAM` を remap 後の値を持つ `RequestStreamEnd::Reset` にし、それ以外のストリームエラーはエラーのまま返すこと
