...
- `src/session/types.rs` の `SessionEvent::StopSendingRequestStream` は `src/` から一度も発行されない。受信方向 cancel は I/O 層主導で完結する。`examples/moqt-transport/src/moqt_client.rs` が match で握り潰すためだけに存在する。
...
- `StopSendingRequestStream`: 公開 enum variant の削除は破壊的変更。削除または `ResetRequestStream` への統合を検討し、`CHANGES.md` に明記する。
