...
自側 publisher 役が送る LARGEST_OBJECT を `publisher_track_largest` (同一 Track の publisher 役 subscription 群の
`effective_largest_object` の最大) で算出するようにした。`publisher_track_largest` は self を不変借用するため、
subscription の可変借用より前に求める形にしている。

- `src/session/subscription/send.rs` の `send_subscribe_ok`: `effective_largest_object(subscription)` を
  `publisher_track_largest(&subscription.track_namespace, &subscription.track_name)` に置き換えた
  (subscription の可変借用の前に算出)
- `src/session/subscription/send.rs` の `send_publish_state_notify`: 同じく `publisher_track_largest` に置き換えた。
  未使用になった `effective_largest_object` の import を削除
- `src/session/subscription/dispatch.rs` の `send_ok_for_subscription`: 自側 publisher 役のときは
  `publisher_track_largest`、自側 subscriber 役のときは従来どおり `effective_largest_object` を使う形に分岐した
  (subscriber 役は peer から受信した値を保持しており、購読横断の集約は対象外)
- `update_largest_object_in_parameters` の「既存値との max を取る」挙動と、`publisher_track_largest` が `None` のときに
  LARGEST_OBJECT を付与しない挙動は変えていない

回帰テストは `tests/test_session/subscription/largest_object_track_aggregate.rs` に 3 本追加した。同一 Track へ 2 本目の
SUBSCRIBE を受理する状況で、1 本目の subscription で `{group, object}` を publish 済みにし、観測値を持たない 2 本目の
SUBSCRIBE_OK / REQUEST_UPDATE_OK / PUBLISH_STATE_NOTIFY に同一 Track の最大値が載ることを検証する。

検証は `cargo test --workspace` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo fmt --all -- --check` の
通過で確認した。CI の test ジョブは prebuilt ライブラリのダウンロードで `curl: (6) Could not resolve host: github.com`
により 1 度失敗したが、`gh run rerun --failed` で解消した (コード起因ではない)。
