...
`examples/moqt-transport/src/moqt_client.rs` の `drain_events` の `SessionEvent::SendOnStream` を、`bidi_sends` にエントリが無い場合は no-op にした。

- `let Some(..) else { return Err(..) }` を `if let Some(..) { ... } else { ... }` に変え、エントリが無い場合は warn ログ
  (`Dropping send on already closed request stream: request_id=.., fin=.., message=..`) を残して同一イベントの処理を終える
- `continue` ではなく if/else にしたのは、`drain_events` の残りのイベント処理 (イベントキューの drain と `cleanup_closed_requests`) を打ち切らないためである
- 送信半が台帳に無い理由 (送信方向が FIN / RESET 済み) と、Session が同一 request へ 2 度目の fin 付き送信を発行し得る経路 (拒否した REQUEST_UPDATE がパイプラインで届く場合) をコメントに明記した。`ResetRequestStream` の「既に閉じた request は no-op」と同じ方針である
- 初回の `SendOnStream` (fin の有無を問わず) の挙動は変えていない。`closed_request_streams` による回収との二重 `finish` も、台帳からエントリを消した側だけが `finish` するため発生しない

検証は `cargo test --workspace` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo fmt --all -- --check` の通過で確認した (example のイベントループは実 WebTransport ストリームを必要とするため単体テストの対象外。Session 側が 2 度目の fin 付き送信を発行することは library のテストで確認済み)。
