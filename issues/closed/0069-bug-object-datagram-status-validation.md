...
`src/session/data.rs` の `send_object_datagram` に status の値域検証を追加した。

- 検証は既存の `properties_data` 検証の直後 (フィルタ評価と `largest_received_location` 更新より前) に行う。
  拒否時は状態を汚染しない (未送信の Object を観測済みにしない)
- 判定は encoder (`ObjectDatagram::encode`) と同じ `crate::stream::validate_object_status` を使う。
  同関数を `pub(crate)` に変更し、session 層と stream 層で 1 箇所の判定を共有する
- 0x0 (Normal) / 0x3 (End of Group) / 0x4 (End of Track) 以外は `SESSION_PROTOCOL_VIOLATION` で拒否する
- `ObjectDatagram::encode` とその他の既存挙動は変えていない

テストは `tests/test_session/data_stream.rs` に 2 本追加した。

- `send_object_datagram_rejected_for_unknown_status`: status 0x5 が `SESSION_PROTOCOL_VIOLATION` で拒否され、
  `largest_received_location` が `None` のままであること
- `send_object_datagram_accepts_known_status_values`: `None` / 0x0 / 0x3 / 0x4 は従来どおり送信でき、
  最大位置が更新されること
