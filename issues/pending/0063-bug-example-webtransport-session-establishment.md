...
`issues/closed/0093-bug-webtransport-settings-order.md` の対応で、本 issue の次の項目は
解消済みである。

- CONNECT 送信前に peer 制御ストリームを受信して SETTINGS を feed する
  (`wait_for_peer_settings` を `send_request` の前に呼ぶ)
- `set_webtransport_transport_verified` を呼ぶ
  (`examples/moqt-transport/src/webtransport.rs` で `(true, false)` を渡す。
  `max_datagram_frame_size` は datagram provider を有効にしていることを根拠に true、
  `reset_stream_at` は s2n-quic が送出しないため false)

残る「確立前後の datagram を失わない」と「`feed_datagram` の drain 分離」は未着手である。

実測 (2026-09-17、relay の WebTransport over HTTP/3、
`https://127.0.0.1:4433/moqt` へ `moqt-subscriber` で接続):

```text
    $ ./target/debug/moqt-subscriber --url https://127.0.0.1:4433/moqt
    WARN TLS certificate verification is disabled (development mode)
    ERROR Fatal: WebTransport: http3 error: webtransport setup error:
          reset_stream_at transport parameter not supported
```

relay は `wt_enabled` (draft-ietf-webtrans-http3-16 相当) を広告するため、
`DraftVersion::requires_reset_stream_at()` が真になり、s2n-quic が RESET_STREAM_AT を
実装していない本 example では CONNECT に到達しない。この依存は
`issues/pending/0094-bug-webtransport-reset-stream-at-unsupported.md` で扱っている。

したがって本 issue の完了条件 (WebTransport 接続の確立と datagram 受信) は、
0094 が解消するまで検証できない。

## pending にする理由

WebTransport セッションの確立が s2n-quic の RESET_STREAM_AT 未実装に阻まれており、
アプリケーション層では解消できない (0094)。0094 を解消した時点で reopened し、
確立前後の datagram 保持と `feed_datagram` の drain 分離を実装して完了条件を確認する。
