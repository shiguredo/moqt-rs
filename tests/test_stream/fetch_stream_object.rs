...
// End of Range の 3 variant は Properties フィールドを持たないため、
// properties_data に Some を渡したら書き込み前に拒否する
// (draft-ietf-moq-transport-21 §11.4.1.2 (End of Range):
// "Subgroup ID, Priority and Properties are not present")
#[test]
fn encode_end_of_range_with_properties_rejected() {
    let entries = [
        FetchStreamEntry::EndOfNonExistentRange {
            group_id: 1,
            object_id: 2,
    ...
    for entry in &entries {
        ...
        assert!(
            ...
            "End of Range に properties_data を渡したら拒否すること: {entry:?}"
        );
        // 拒否した場合は書き込みを行わない
        assert_eq!(buf, vec![0xAA], "失敗時に buf が変化しないこと: {entry:?}");
    }
}

// End of Range の 3 variant は properties_data = None なら従来どおり成功する
#[test]
fn encode_end_of_range_without_properties_succeeds() {
    let entries = [
        FetchStreamEntry::EndOfNonExistentRange {
            group_id: 1,
            object_id: 2,
    ...
    for entry in &entries {
        ...
        entry
            .encode(None, FetchPriorContext::HasPriorObject, &mut buf)
            .expect("None は常に成功すること");
        assert!(!buf.is_empty(), "エンコード結果が空にならないこと");
        let (decoded, consumed) = FetchStreamEntry::decode(&buf, FetchPriorContext::HasPriorObject)
            .expect("エンコード結果をデコードできること");
        assert_eq!(consumed, buf.len());
