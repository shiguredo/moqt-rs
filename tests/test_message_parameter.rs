...
/// draft-ietf-moq-transport-21 §8.9 / §9.1.4 (Authorization Token) のエラー経路。
///
/// §8.9: "If the Token structure cannot be decoded, the receiver MUST close the
/// Session with KEY_VALUE_FORMATTING_ERROR."
mod authorization_token_errors {
    use super::*;
    use shiguredo_moqt::message_parameter::PARAM_AUTHORIZATION_TOKEN;

    /// AUTHORIZATION_TOKEN (0x03) の 1 パラメータだけを持つ Parameters を手で組み立てる
    ///
    /// ワイヤは count=1, delta=0x03, value length, value の順である。
    fn parameters_with_token_value(value: &[u8]) -> Vec<u8> {
        let mut buf = vec![0x01, PARAM_AUTHORIZATION_TOKEN as u8, value.len() as u8];
        buf.extend_from_slice(value);
        buf
    }

    #[test]
    fn delete_with_trailing_bytes_is_key_value_formatting_error() {
        // DELETE (0x0) は Token Alias だけを運ぶ。余剰バイトがあれば Token 構造として
        // デコードできないため KEY_VALUE_FORMATTING_ERROR (§8.9 MUST)
        let buf = parameters_with_token_value(&[0x00, 0x01, 0xFF]);
        assert!(
            matches!(
                MessageParameters::decode(&buf),
                Err(MessageError::KeyValueFormattingError(_))
            ),
            "DELETE の余剰バイトは KEY_VALUE_FORMATTING_ERROR であること"
        );
    }

    #[test]
    fn use_alias_with_trailing_bytes_is_key_value_formatting_error() {
        // USE_ALIAS (0x2) も Token Alias だけを運ぶ
        let buf = parameters_with_token_value(&[0x02, 0x01, 0xFF]);
        assert!(
            matches!(
                MessageParameters::decode(&buf),
                Err(MessageError::KeyValueFormattingError(_))
            ),
            "USE_ALIAS の余剰バイトは KEY_VALUE_FORMATTING_ERROR であること"
        );
    }

    #[test]
    fn unknown_alias_type_is_key_value_formatting_error() {
        // Alias Type は 0x0 (DELETE) / 0x1 (REGISTER) / 0x2 (USE_ALIAS) /
        // 0x3 (USE_VALUE) のみ定義されている。0x04 は未知である
        let buf = parameters_with_token_value(&[0x04, 0x01]);
        assert!(
            matches!(
                MessageParameters::decode(&buf),
                Err(MessageError::KeyValueFormattingError(_))
            ),
            "未知の Alias Type は KEY_VALUE_FORMATTING_ERROR であること"
        );
    }

    #[test]
    fn delete_without_alias_is_key_value_formatting_error() {
        // DELETE の Token Alias が欠けている場合も KEY_VALUE_FORMATTING_ERROR
        let buf = parameters_with_token_value(&[0x00]);
        assert!(
            matches!(
                MessageParameters::decode(&buf),
                Err(MessageError::KeyValueFormattingError(_))
            ),
            "Token Alias 欠落は KEY_VALUE_FORMATTING_ERROR であること"
        );
    }

    #[test]
    fn encode_rejects_token_value_over_65535_bytes() {
        // draft-ietf-moq-transport-21 §8.3 (Key-Value-Pair Structure):
        // 値長上限は 2^16-1 バイト。REGISTER の Token Value で上限を超えると拒否する
        let mut params = MessageParameters::new();
        params.push(MessageParameter {
            param_type: PARAM_AUTHORIZATION_TOKEN,
        ...
        assert_eq!(
            ...
            "65536 バイトの Token Value は encode できないこと"
        );
    }

    #[test]
    fn encode_accepts_token_value_at_65535_bytes() {
        // 上限ちょうどは受理される (境界)
        // Token Alias Type (1) + Token Alias (1) + Token Type (1) を引く
        let token_value_len = 65535 - 3;
        let mut params = MessageParameters::new();
        params.push(MessageParameter {
            param_type: PARAM_AUTHORIZATION_TOKEN,
            value: MessageParameterValue::AuthorizationToken(AuthorizationToken::Register {
                alias: 1,
                token_type: 0,
                token_value: vec![0xAB; token_value_len],
            }),
        });
        let mut buf = Vec::new();
        params
            .encode(&mut buf)
            .expect("上限ちょうどの Token Value は encode できる");
        let (decoded, _consumed) =
            MessageParameters::decode(&buf).expect("encode した Parameters は decode できる");
        assert_eq!(decoded.authorization_tokens().len(), 1);
    }

    #[test]
    fn encode_rejects_length_prefixed_value_over_65535_bytes() {
        // LengthPrefixed でも同じ上限が適用される
        let mut params = MessageParameters::new();
        params.push(MessageParameter {
            param_type: PARAM_TRACK_PROPERTY_FILTER,
            value: MessageParameterValue::LengthPrefixed(vec![0xAB; 65536]),
        });
        let mut buf = Vec::new();
        assert_eq!(
            params.encode(&mut buf),
            ...
            "65536 バイトの LengthPrefixed は encode できないこと"
        );
    }
}
