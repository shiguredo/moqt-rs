...
/// FIN で閉じた subgroup stream に後から届く RESET_STREAM を無視する
///
/// draft-ietf-moq-transport-21 §11.3.2 (Closing Subgroup Streams): "A sender might send all
/// objects in a Subgroup and the FIN on a QUIC stream, and then reset the stream. In this
/// case, the receiving application would receive the FIN if and only if all objects were
/// received. If the application receives all data on the stream and the FIN, it can ignore
/// any subsequent reset."
///
/// QUIC では FIN 送信後も RESET_STREAM を送れるため、FIN の終端通知の後に同じ stream id の
/// RESET が届いてもセッションを閉じない。
#[test]
fn reset_after_fin_on_subgroup_stream_is_ignored() {
    let (mut client, _server, _rid) = establish_subscribe_track(700);
    let stream_id = DataStreamId(10);

    // publisher が subgroup stream を開いて 1 object 送り、FIN で閉じる
    let header = SubgroupHeader {
        track_alias: 700,
        group_id: 1,
        subgroup_id: SubgroupIdMode::Explicit(0),
    ...
    client
        .recv_data_stream_closed(stream_id, RequestStreamEnd::Fin)
        .expect("FIN の終端通知は受理される");

    // FIN の後に同じ stream の RESET_STREAM が届いても無視する
    client
        .recv_data_stream_closed(
            stream_id,
            RequestStreamEnd::Reset {
        ...
        .expect("FIN 後の reset は無視される");
    assert_eq!(
        client.state(),
        SessionState::Established,
        "FIN 後の reset でセッションを閉じない"
    );
}

/// reset だけを受けた stream への再終端通知は従来どおりプロトコル違反にする
///
/// FIN を受けていない stream の終端を再度通知するのは、対応する stream が存在しない
/// 終端通知であり、未知 stream id の終端として扱う (誤りの検出を弱めない)。
#[test]
fn second_close_after_reset_is_still_protocol_violation() {
    let (mut client, _server, _rid) = establish_subscribe_track(700);
    let stream_id = DataStreamId(11);
    ...
    client
        ...
        .expect("reset の終端通知は受理される");

    let err = client
        .recv_data_stream_closed(stream_id, RequestStreamEnd::Fin)
        .expect_err("reset 済み stream への再終端通知は拒否される");
    assert_eq!(err.code, SESSION_PROTOCOL_VIOLATION);
}

/// 既知でない status (0x5) は wire 生成前に拒否され、最大位置も更新されない
///
...
fn send_object_datagram_rejected_for_unknown_status() {
    ...
    // 拒否時は状態を汚染しない (未送信の Object を観測済みにしない)
    let sub = server.subscription(rid).expect("subscription が存在する");
    assert_eq!(sub.largest_received_location, None);
}

/// 既知 status (0x0 / 0x3 / 0x4) と None は従来どおり送信でき、最大位置が更新される
#[test]
fn send_object_datagram_accepts_known_status_values() {
    for status in [None, Some(0x0), Some(0x3), Some(0x4)] {
        ...
        assert_eq!(
            ...
            "status={status:?} でも最大位置は更新される"
        );
    }
}

// ============================================================================
// recv_object_datagram の入口検証 (draft-ietf-moq-transport-21 §11.1.2 / §11.1.3 / §11.2.1)
// ============================================================================
//
// wire 経路 (`recv_datagram` → `ObjectDatagram::decode`) が行う検証を、decode 済みまたは
// 手組みの `ObjectDatagram` を受け取る `recv_object_datagram` の入口でも行い、
// 違反時は wire 経路と同じ SESSION_PROTOCOL_VIOLATION でセッションを閉じる。

/// 検証用の properties blob (Properties Length + LOC TIMESTAMP) を作る
fn valid_datagram_properties_blob() -> Vec<u8> {
    let mut props = LocProperties::new();
    props.push(LocProperty {
        prop_id: PROP_TIMESTAMP,
        value: LocPropertyValue::VarInt(42),
    });
    props.encode().expect("正当な LOC properties である")
}

/// 未知 status (0x5) の hand-built datagram は拒否され、購読状態も更新されない
#[test]
fn recv_object_datagram_rejects_unknown_status() {
    let (mut client, _, rid) = establish_subscribe_track(520);
    let err = client
        .recv_object_datagram(&ObjectDatagram {
            track_alias: 520,
            group_id: 3,
    ...
    assert_eq!(
        client
            ...
            .largest_received_location,
        None,
        "拒否時に subscription 状態を汚染しない"
    );
}

/// STATUS と END_OF_GROUP の同時指定は拒否される
#[test]
fn recv_object_datagram_rejects_status_with_end_of_group() {
    let (mut client, _, _) = establish_subscribe_track(521);
    let err = client
        .recv_object_datagram(&ObjectDatagram {
            track_alias: 521,
            group_id: 3,
        ...
        .expect_err("STATUS と END_OF_GROUP の同時指定は拒否される");
    assert_eq!(err.code, SESSION_PROTOCOL_VIOLATION);
}

/// 非 Normal status への Properties 付与は拒否される
#[test]
fn recv_object_datagram_rejects_properties_on_non_normal_status() {
    let (mut client, _, _) = establish_subscribe_track(522);
    let err = client
        .recv_object_datagram(&ObjectDatagram {
            track_alias: 522,
            group_id: 3,
            object_id: 1,
            publisher_priority: Some(7),
            properties_data: Some(valid_datagram_properties_blob()),
            end_of_group: false,
            // End of Track (0x4)
            status: Some(0x4),
        })
        .expect_err("非 Normal status の Properties は拒否される");
    assert_eq!(err.code, SESSION_PROTOCOL_VIOLATION);
}

/// Properties blob の宣言長と実データ長の不一致は拒否される
#[test]
fn recv_object_datagram_rejects_malformed_properties_blob() {
    let (mut client, _, _) = establish_subscribe_track(523);
    let err = client
        .recv_object_datagram(&ObjectDatagram {
            track_alias: 523,
            group_id: 3,
            object_id: 1,
            publisher_priority: Some(7),
            // 宣言長 2 に対してデータが 1 バイトしかない
            properties_data: Some(vec![0x02, 0x01]),
            end_of_group: false,
            status: None,
        })
        .expect_err("Properties blob の長さ不一致は拒否される");
    assert_eq!(err.code, SESSION_PROTOCOL_VIOLATION);
}

/// Properties Length = 0 の blob は拒否される
#[test]
fn recv_object_datagram_rejects_zero_length_properties_blob() {
    let (mut client, _, _) = establish_subscribe_track(524);
    let err = client
        .recv_object_datagram(&ObjectDatagram {
            track_alias: 524,
            group_id: 3,
        ...
        .expect_err("Properties Length = 0 は拒否される");
    assert_eq!(err.code, SESSION_PROTOCOL_VIOLATION);
}
