...
///
/// 初回の SUBSCRIBE_NAMESPACE と同じ条件を更新経路でも検証し、DOES_NOT_EXIST の
/// REQUEST_ERROR を FIN 付きで返す。prefix と subscription state は変わらず、
/// セッションも閉じない。
#[test]
fn subscribe_namespace_update_prefix_single_period_rejected() {
    use shiguredo_moqt::error::REQUEST_DOES_NOT_EXIST;
    use shiguredo_moqt::message_parameter::{
        MessageParameter, MessageParameterValue, PARAM_TRACK_NAMESPACE_PREFIX,
    };
    use shiguredo_moqt::session::types::NamespaceSubscriptionState;
    let old_prefix = ns(&[b"example"]);
    let (_client, mut server, rid) = establish_subscribe_namespace_with(old_prefix.clone());
...
/// セッションも閉じない。
#[test]
fn subscribe_namespace_update_prefix_session_level_rejected() {
    use shiguredo_moqt::error::REQUEST_DOES_NOT_EXIST;
    use shiguredo_moqt::message_parameter::{
        MessageParameter, MessageParameterValue, PARAM_TRACK_NAMESPACE_PREFIX,
    };
    ...
    match err_msg {
        ControlMessage::RequestError(e) => {
            assert_eq!(e.error_code, REQUEST_DOES_NOT_EXIST);
            assert_eq!(e.reason.as_str(), "session-level namespace does not exist");
        }
        _ => panic!("DOES_NOT_EXIST の RequestError が期待される"),
    }
    assert_eq!(
...
#[test]
fn subscribe_namespace_update_prefix_reserved_rejected_locally() {
