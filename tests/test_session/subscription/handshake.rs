...
/// responder の FIN を受けた requester に FinishRequestStream を発行する
///
/// draft-ietf-moq-transport-21 §6.4.2.2 (Graceful Request Stream Closure): "A FIN sent by
/// the responder after its response and any subsequent messages for the request signals that
/// the request is complete; if it has not already done so, the requester SHOULD then send a
/// FIN on its direction, gracefully closing the stream."
#[test]
fn requester_gets_finish_request_stream_on_responder_fin() {
    let (mut client, mut server) = establish_pair();
    let rid = client
        .send_subscribe(ns(&[b"live"]), b"cam1".to_vec(), MessageParameters::new())
        .expect("テストフィクスチャの前提条件を満たす");
    ...
    // responder (publisher 役) が FIN で応答を終える
    client
    ...
    assert!(
        events.iter().any(|ev| matches!(
            ev,
            SessionEvent::FinishRequestStream { request_id } if *request_id == rid
        )),
        "requester に FinishRequestStream が発行される: {events:?}"
    );
}

/// responder 側では responder の FIN 相当で FinishRequestStream を発行しない
///
/// 自側が responder (publisher 役) の場合、PUBLISH_DONE を送ってから FIN する義務があるため、
/// peer (requester) の FIN を受けた時点で送信方向を閉じてはならない。
#[test]
fn responder_does_not_get_finish_request_stream() {
    let (mut client, mut server) = establish_pair();
    let rid = client
        .send_subscribe(ns(&[b"live"]), b"cam1".to_vec(), MessageParameters::new())
        .expect("テストフィクスチャの前提条件を満たす");
    ...
    // requester (subscriber 役) が FIN で要求を終える
    server
        .recv_request_stream_closed(rid, RequestStreamEnd::Fin)
        .expect("テストフィクスチャの前提条件を満たす");
    ...
    assert!(
        !events
            .iter()
            .any(|ev| matches!(ev, SessionEvent::FinishRequestStream { .. })),
        "responder には FinishRequestStream を発行しない: {events:?}"
    );
}
